...
void getSigKernelPreferSpeed_(
	...
	const uint64_t difflength1 = dimension * (length1 - 1);
	const uint64_t difflength2 = dimension * (length2 - 1);

	double* diff1 = (double*)malloc(difflength1 * sizeof(double));
	if (!diff1) {
		throw std::bad_alloc();
		return;
	}
	double* diff2 = (double*)malloc(difflength2 * sizeof(double));
	if (!diff2) {
		throw std::bad_alloc();
		return;
	}
	...
	//Populate diffs
	for (uint64_t i = 0; i < difflength1; ++i) {
		diff1[i] = static_cast<double>(path1[i + dimension]) - static_cast<double>(path1[i]);
	}
	for (uint64_t i = 0; i < difflength2; ++i) {
		diff2[i] = static_cast<double>(path2[i + dimension]) - static_cast<double>(path2[i]);
	}
	...
	double* diff1Ptr = diff1;

	for (uint64_t ii = 0; ii < length1 - 1; diff1Ptr += dimension, ++ii) {
		double* diff2Ptr = diff2;
		T* prevPt2(path2);
		T* nextPt2(path2);
		nextPt2 += dimension;
		for (uint64_t m = 0; m < length2 - 1; diff2Ptr += dimension, ++m) {
			double deriv = dot_product(diff1Ptr, diff2Ptr, dimension);
			deriv *= dyadicFrac;
			double deriv2 = deriv * deriv * twelth;
...
void sigKernel_(
	...
	if (preferSpeedOverMemory) {
		getSigKernelPreferSpeed_(path1, path2, length1, length2, dimension, out, dyadicOrder1, dyadicOrder2);
	}
	else {
		getSigKernelPreferMemory_(path1, path2, length1, length2, dimension, out, dyadicOrder1, dyadicOrder2);
	}
}
...
void batchSigKernel_(
	...
	if (preferSpeedOverMemory) {
		sigKernelFunc = [&](T* path1Ptr, T* path2Ptr, double* outPtr) {
			getSigKernelPreferSpeed_(path1Ptr, path2Ptr, length1, length2, dimension, outPtr, dyadicOrder1, dyadicOrder2);
			};
	}
	else {
		sigKernelFunc = [&](T* path1Ptr, T* path2Ptr, double* outPtr) {
			getSigKernelPreferMemory_(path1Ptr, path2Ptr, length1, length2, dimension, outPtr, dyadicOrder1, dyadicOrder2);
			};
	}
