...
void batchSignature(double* path, double* out, uint64_t batchSize, uint64_t dimension, uint64_t length, uint64_t degree, bool timeAug, bool leadLag, bool horner, bool parallel) {
	batchSignature_<double>(path, out, batchSize, dimension, length, degree, timeAug, leadLag, horner, parallel);
}

void batchSignatureInt(int* path, double* out, uint64_t batchSize, uint64_t dimension, uint64_t length, uint64_t degree, bool timeAug, bool leadLag, bool horner, bool parallel) {
	batchSignature_<int>(path, out, batchSize, dimension, length, degree, timeAug, leadLag, horner, parallel);
}