...
template<typename T>
void transform_path_(T* data_in, double* data_out, const uint64_t dimension, const uint64_t length, bool time_aug, bool lead_lag) {
	Path<T> path(data_in, dimension, length, time_aug, lead_lag);
	...
	for (Point<T> pt = path.begin(); pt != end_pt; ++pt) {
		for (uint64_t i = 0; i < transformed_dimension; ++i) {
			(*out_ptr) = static_cast<double>(pt[i]);
			++out_ptr;
		}
	}
}
...
template<typename T>
void batch_transform_path_(T* data_in, double* data_out, const uint64_t batch_size, const uint64_t dimension, const uint64_t length, bool time_aug, bool lead_lag, int n_jobs)
{
	...
	transform_func = [&](T* path_ptr, double* out_ptr) {
		transform_path_<T>(path_ptr, out_ptr, dimension, length, time_aug, lead_lag);
		};
