...
from .dtypes import CPSIG_SIGNATURE, CPSIG_BATCH_SIGNATURE, CPSIG_LOG_SIGNATURE, CPSIG_BATCH_LOG_SIGNATURE
...
def log_sig_combine(
    ...
    """
    Combines two truncated log-signatures of the same degree and dimension into one log-signature. In particular, let
    :math:`x_1, x_2` be two paths such that the first point of :math:`x_2` is the last point of :math:`x_1`. Let
    :math:`\\log S(x_1), \\log S(x_2)` be the truncated log-signatures of :math:`x_1, x_2` respectively. Then calling this
    function on :math:`\\log S(x_1), \\log S(x_2)` returns the truncated log-signature of the concatenated path,

    .. math::

        \\log S(x_1 * x_2) = \\log(\\exp(\\log S(x_1)) \\otimes \\exp(\\log S(x_2))).

    :param log_sig1: The first truncated log-signature
    :type log_sig1: numpy.ndarray | torch.tensor
    :param log_sig2: The second truncated log-signature. Must have the same degree and dimension as the first.
    ...
    :param n_jobs: Number of threads to run in parallel. If n_jobs = 1, the computation is run serially.
        ...
        threads are used. For example if n_jobs = -2, all threads but one are used.
    :type n_jobs: int
    :return: Combined log-signature, :math:`\\log S(x_1 * x_2)`
    :rtype: numpy.ndarray | torch.tensor
    """
    check_type(dimension, "dimension", int)
    check_type(degree, "degree", int)
    ...
    device_handler = DeviceToHost([log_sig1, log_sig2], ["log_sig1", "log_sig2"])
    log_sig1, log_sig2 = device_handler.data

    log_sig_len = log_sig_length(dimension, degree)
    data = DoubleSigInputHandler(log_sig1, log_sig2, log_sig_len, "log_sig1", "log_sig2")
    result = SigOutputHandler(data, log_sig_len)

    if data.is_batch:
        err_code = CPSIG.batch_log_sig_combine(
            data.sig1_ptr,
            data.sig2_ptr,
            result.data_ptr,
            data.batch_size,
        ...
        err_code = CPSIG.log_sig_combine(
            data.sig1_ptr,
            data.sig2_ptr,
            result.data_ptr,
            dimension,
...
def log_signature_(data, result, degree, horner = True):
...
def batch_log_signature_(data, result, degree, horner = True, n_jobs = 1):
...
def log_signature(
    ...
    """
    Computes the truncated log-signature of a single path or a batch of paths. For a single path :math:`x`, the
    log-signature is the tensor logarithm of its signature,

    .. math::

        \\log S(x)_{[s,t]} := \\log\\left( 1, S(x)^{(1)}_{[s,t]}, \\ldots, S(x)^{(N)}_{[s,t]}\\right).

    :param path: The underlying path or batch of paths, given as a `numpy.ndarray` or `torch.tensor`.
        For a single path, this must be of shape ``(length, dimension)``. For a batch of paths, this must
        be of shape ``(batch_size, length, dimension)``.
    :type path: numpy.ndarray | torch.tensor
    :param degree: The truncation level of the log-signature, :math:`N`.
    :type degree: int
    ...
    :param end_time: End time for time-augmentation, :math:`t_L`.
    :type end_time: float
    :param horner: If True, will use Horner's algorithm for the intermediate polynomial multiplications.
    :type horner: bool
    :param n_jobs: Number of threads to run in parallel. If n_jobs = 1, the computation is run serially.
        If set to -1, all available threads are used. For n_jobs below -1, (max_threads + 1 + n_jobs)
        threads are used. For example if n_jobs = -2, all threads but one are used.
    :type n_jobs: int
    :return: Truncated log-signature, or a batch of truncated log-signatures.
    :rtype: numpy.ndarray | torch.tensor
    """
    check_type(degree, "degree", int)
    check_type(horner, "horner", bool)
    ...
    data = PathInputHandler(path, time_aug, lead_lag, end_time, "path")
    log_len = log_sig_length(data.dimension, degree)
    result = SigOutputHandler(data, log_len)
    if data.is_batch:
        check_type(n_jobs, "n_jobs", int)
        if n_jobs == 0:
            raise ValueError("n_jobs cannot be 0")
        res = batch_log_signature_(data, result, degree, horner, n_jobs)
    else:
        res = log_signature_(data, result, degree, horner)
